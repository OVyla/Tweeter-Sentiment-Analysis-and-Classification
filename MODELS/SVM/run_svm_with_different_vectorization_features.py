...
from MODELS.SVM.svm_model import svm_one_vs_one
...
log_file = os.path.join(log_dir, "output_features_tfidf.txt")       # /MODELS/LogisticRegression/output.txt
...
def main():
    ...
    models_to_test = [
        ("svmovo", svm_one_vs_one)
    ]
    vectorizers_to_test = ['TFIDF']
    max_features_list = [500, 1000, 2000, 5000, 10000, 20000, 40000, 80000, 130000, 160000]

    # --- Main Benchmark Loop ---
    for vec_method in vectorizers_to_test:
        for max_features in max_features_list:
            data = load_and_vectorize_splits(method=vec_method, max_features=max_features)
            ...
            for model_name, model_fn in models_to_test:
                print(f"\n============================================================")
                print(f"Model: {model_name} | Vectorització: {vec_method} | Max Features: {max_features}")
                print(f"============================================================\n")
