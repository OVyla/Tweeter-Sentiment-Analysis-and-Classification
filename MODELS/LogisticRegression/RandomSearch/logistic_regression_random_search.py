...
# CONFIGURACIÓ
# ==========================================
OUTPUT_FILE = "lr_random_search_results.txt"
CSV_RESULTS = "lr_random_search_results.csv"
N_ITER = 15  # Número de combinaciones aleatorias a probar (reducido)
SAMPLE_SIZE = 0.1  # 10% del dataset para tuning (MUCHO más rápido)
CV_FOLDS = 2
RANDOM_STATE = 42
...
def main():
    print("\n" + "="*80)
    print("LOGISTIC REGRESSION - RANDOM SEARCH (TODOS LOS PARÁMETROS)")
    print("="*80)
    ...
    param_dist = {
        'C': np.logspace(-3, 3, 15),  # [0.001, 0.01, 0.1, 1, 10, 100, 1000]
        'penalty': ['l1', 'l2'],
        'solver': ['liblinear', 'saga'],
        'max_iter': [100, 300, 500, 1000, 2000],
        'class_weight': [None, 'balanced'],
        'fit_intercept': [True, False],
        'intercept_scaling': [0.1, 0.5, 1, 2, 10],
    ...
    # 5. Entrenar modelo final con 100% del dataset usando los mejores parámetros
    print("\n[5/5] Entrenando modelo final con 100% del dataset...")
    final_model = LogisticRegression(**rs.best_params_, random_state=RANDOM_STATE, n_jobs=-1)
    ...
    with open(OUTPUT_FILE, "w") as f:
        f.write("="*80 + "\n")
        f.write("LOGISTIC REGRESSION - RANDOM SEARCH (TODOS LOS PARÁMETROS)\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"\n  CV Score (30% dataset): {rs.best_score_:.4f}\n")
    ...
    print("RESUMEN FINAL")
    print("="*80)
    print(f"CV Score (30% dataset): {rs.best_score_:.4f}")
