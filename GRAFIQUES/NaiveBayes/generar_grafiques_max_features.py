...
OUTPUT_DIR = os.path.join(ROOT_DIR, "GRAFIQUES", "NaiveBayes")
...
def plot_validation_accuracy(max_features, val_acc, output_dir):
    ...
    plt.title("Naive Bayes TF-IDF: Accuracy de validació vs max_features")
    plt.grid(True, which="both", linestyle="--", alpha=0.5)

    out_path = os.path.join(output_dir, "nb_tfidf_val_accuracy_vs_max_features.png")
    plt.tight_layout()
...
def plot_training_time(max_features, train_time, output_dir):
    ...
    plt.title("Naive Bayes TF-IDF: Temps d'entrenament vs max_features")
    plt.grid(True, which="both", linestyle="--", alpha=0.5)
